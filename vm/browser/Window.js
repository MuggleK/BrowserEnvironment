...
var windowPrototype =  new WindowProperties();
//伪造构造函数和名字
...
//生成一个window
var Window = function Window(){};   safefunction(Window);
var window = this;   // 遇到检测node需hook调检测函数  例如：TDC检测node下的process
// var window = global;
...
window["fetch_data"] = new (class FetchData {});
window["fetch"] = function fetch(xx, yy){
    debugger;
    window["fetch_data"]['url'] = xx;
    window["fetch_data"]['data'] = yy["body"];
    const fs = require('fs');

    fs.writeFile('fetch_data.txt', JSON.stringify(window["fetch_data"]), err => {
        if (err) {
            console.error(err);
        }
    });
    return window["fetch_data"];
...
window["chrome"]['csi'] = function csi(xx,yy){debugger;};   safefunction(window["chrome"]['csi']);
window["chrome"]['loadTimes'] = function loadTimes(xx,yy){debugger;};   safefunction(window["chrome"]['loadTimes']);
window["chrome"]['runtime'] = {
    ...
    id: undefined
};  safefunction(window["chrome"]['runtime']);
...
window['WebKitMutationObserver'] = function WebKitMutationObserver(xx){
    return {
        observe: function observe(xx, yy){},
        disconnect: function disconnect(xx, yy){},
        takeRecords: function takeRecords(xx, yy){},
    };
};
window['PerformanceObserver'] = new (class PerformanceObserver{});
